...
void getInitCons(double gamma, vector<vector<vector<double> > >& U0, vector<vector<vector<double> > >& V0, vector<vector<double> >& x_cell, vector<vector<double> >& y_cell);
...
int main() {
    ...
    // Open file
    std::ifstream inFile("../Project_Files/Grids/curviliniear-grids/curv2d9.grd");
    ...
    // ******** Initial Conditions ********
    getInitCons(gamma, U0, V0, x_cell, y_cell); // calculate initial primitive & conservative variables; Have checked correctness!
    ...
    for (int i = 0; i < num_iter; i++){
        ...
        // setBounCond(gamma, V1, U1, VLeft, VRight, VBottom, VTop); // get primitive at ghost cell (including limit range of primitive variables and update conservative)
        // primToLimiter(V1, phiplus_x, phiminus_x, phiplus_y, phiminus_y); 
        ...
        // consToPrim(gamma, U0, V0);
        // setBounCond(gamma, V0, U0, VLeft, VRight, VBottom, VTop);
...
{    
    ...
    for (int i = 2; i < V0[0].size()-2; i++){
        for (int j = 2; j < V0[0][i].size()-2; j++){
            x = x_cell[i-2][j-2];
            y = y_cell[i-2][j-2];
            // primitive variable at cell centers
            V0[0][i][j] = rho0 + rhoy*cos((Pi*y)/(2.*L)) + rhox*sin((Pi*x)/L);
            V0[1][i][j] = uvel0 + uvely*cos((3.*Pi*y)/(5.*L)) + uvelx*sin((3.*Pi*x)/(2.*L));
            V0[2][i][j] = vvel0 + vvelx*cos((Pi*x)/(2.*L)) + vvely*sin((2.*Pi*y)/(3.*L));
            V0[3][i][j] = press0 + pressx*cos((2.*Pi*x)/L) + pressy*sin((Pi*y)/L);
            U0[0][i][j] = V0[0][i][j];
...
void setBounCond(double gamma, vector<vector<vector<double> > >& V0, vector<vector<double> >& VLeft, vector<vector<double> >& VRight, vector<vector<double> >& VBottom, vector<vector<double> >& VTop){
    ...
    for (int d = 0; d < V0.size(); d++){
        for (int j = 2; j < V0[0][0].size()-2; j++){
            ...
            V0[d][Nx-2][j] = 2.0*VRight[d][j-2] - V0[d][Nx-3][j];
    ...
    for (int d = 0; d < V0.size(); d++){
        for (int i = 2; i < V0[0].size()-2; i++){
            ...
            V0[d][i][Ny-2] = 2.0*VTop[d][i-2] - V0[d][i][Ny-3];
