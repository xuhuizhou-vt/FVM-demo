...
//                       using the finite volume method with upwind schemes.
...
int main() {
    ...
    // ofstream writeResi("2nd-order-shock/Van-Leer/"+fileResi); // write residual with iteration
    ...
    // ofstream writePrim("2nd-order-shock/Van-Leer/"+fileV);
    ...
    // ofstream writeCons("2nd-order-shock/Van-Leer/"+fileU);
    ...
    // ofstream writeMach("2nd-order-shock/Van-Leer/"+fileMa); // write Mach number
    ofstream writeMach("2nd-order-shock-diff-kappa/Van-Leer/"+fileMa); // write Mach number
    ...
    ofstream writeX("2nd-order-shock-diff-kappa/Van-Leer/"+fileXCell); // write x cell
    ...
    // ofstream writeExactPrim("2nd-order-shock/Van-Leer/"+fileVExact);
    ...
    // ofstream writeExactCons("2nd-order-shock/Van-Leer/"+fileUExact);
    ...
    // ofstream writeExactMach("2nd-order-shock/Van-Leer/"+fileMaExact); // write Mach number
...
// Functions: To get exact solution
double func(double M, double gamma, double Abar) {
...
void primToFlux(vector<vector<double> >& VL, vector<vector<double> >& VR, vector<vector<double> >& F0, double gamma)
{
    double a_L, a_R, ht_L, ht_R, M_L, M_R, M_plus, M_minus, beta_L, beta_R, alpha_plus, alpha_minus, c_plus, c_minus, pbar_plus, pbar_minus, d_plus, d_minus;
    for (int j = 0; j < F0[0].size(); j++){
        a_L = sqrt(gamma*VL[2][j]/(VL[0][j]+1e-16));
        a_R = sqrt(gamma*VR[2][j]/(VR[0][j]+1e-16));
        ht_L = gamma/(gamma-1.0)*VL[2][j]/(VL[0][j]+1e-16) + 0.5*pow(VL[1][j],2.0);
        ht_R = gamma/(gamma-1.0)*VR[2][j]/(VR[0][j]+1e-16) + 0.5*pow(VR[1][j],2.0);
        M_L = VL[1][j]/(sqrt(gamma*VL[2][j]/(VL[0][j]+1e-16))+1e-16);
        M_R = VR[1][j]/(sqrt(gamma*VR[2][j]/(VR[0][j]+1e-16))+1e-16);
        M_plus = 0.25*pow((M_L+1.0),2.0);
        M_minus = -0.25*pow((M_R-1.0),2.0);
        beta_L = -1.0 * max(0.0, 1.0-trunc(fabs(M_L)));
        beta_R = -1.0 * max(0.0, 1.0-trunc(fabs(M_R)));
        alpha_plus = 0.5*(1.0+copysign(1.0,M_L));
        alpha_minus = 0.5*(1.0-copysign(1.0,M_R));
        c_plus = alpha_plus*(1.0+beta_L)*M_L - beta_L*M_plus;
        c_minus = alpha_minus*(1.0+beta_R)*M_R - beta_R*M_minus;
        pbar_plus = M_plus*(-M_L+2.0);
        pbar_minus = M_minus*(-M_R-2.0);
        d_plus = alpha_plus*(1+beta_L) - beta_L*pbar_plus;
        d_minus = alpha_minus*(1+beta_R) - beta_R*pbar_minus;
        
        F0[0][j] = VL[0][j]*a_L*c_plus + VR[0][j]*a_R*c_minus;
        F0[1][j] = VL[0][j]*a_L*c_plus*VL[1][j] + VR[0][j]*a_R*c_minus*VR[1][j] + d_plus*VL[2][j] + d_minus*VR[2][j];
        F0[2][j] = VL[0][j]*a_L*c_plus*ht_L + VR[0][j]*a_R*c_minus*ht_R;
    }
}
